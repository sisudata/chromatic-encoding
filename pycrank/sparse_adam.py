"""
Pytorch's sparse adam, but with amsgrad.
"""
import math
import torch


class SparseAdamAMS(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                old_max_exp_avg_sq_values = max_exp_avg_sq.sparse_mask(grad)._values()
                max_exp_avg_sq_update_values = torch.maximum(
                    old_max_exp_avg_sq_values,
                    exp_avg_sq_update_values)
                max_exp_avg_sq.add_(make_sparse(max_exp_avg_sq_update_values - old_max_exp_avg_sq_values))
                del old_max_exp_avg_sq_values
                
                # Dense addition again is intended, avoiding another sparse_mask
                numer = exp_avg_update_values.add_(old_exp_avg_values)
                exp_avg_sq_update_values.add_(old_exp_avg_sq_values)
                denom = max_exp_avg_sq_update_values.sqrt_().add_(group['eps'])
